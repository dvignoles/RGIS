...
static void _UITableFieldSelectCBK(Widget widget, DBInt select, XmAnyCallbackStruct *callData) {
    ...
    if (dShell == (Widget) NULL) {
        ...
        button = XtVaCreateManagedWidget("UITableSelectCondButton", xmPushButtonGadgetClass, condMenu,
                                         ...
                                         XmNuserData, &cond,
                                         NULL);
        XtAddCallback(button, XmNactivateCallback, (XtCallbackProc) _UITableFieldConditionCBK,
                      (XtPointer) ((char *) NULL + cond++));
        XmStringFree(string);
        string = XmStringCreate((char *) ">=", UICharSetNormal);
        button = XtVaCreateManagedWidget("UITableSelectCondButton", xmPushButtonGadgetClass, condMenu,
                                         XmNlabelString, string,
        ...
        XtAddCallback(UIDialogFormGetOkButton(dShell), XmNactivateCallback, (XtCallbackProc) UIAuxSetBooleanTrueCBK,
                      &proc);
        XtAddCallback(UIDialogFormGetCancelButton(dShell), XmNactivateCallback,
                      (XtCallbackProc) UIAuxSetBooleanFalseCBK, &proc);
        ...
        if ((field = dbTable->Field(text)) == (DBObjTableField *) NULL)
            switch (dbField->Type()) {
                case DBTableFieldInt:
                    if (sscanf(text, "%d", &intVal) != 1) {
                        UIMessage((char *) "Invalid Integer Value");
                        goto Stop;
                    }
                    break;
                case DBTableFieldFloat:
                    if (sscanf(text, "%lf", &floatVal) != 1) {
                        UIMessage((char *) "Invalid Float Value");
                        goto Stop;
                    }
                    break;
...
static void _UITableFieldUniqueCBK(Widget widget, void *data, XmAnyCallbackStruct *callData) {
    ...
    switch (dbField->Type()) {
        ...
        case DBTableFieldString:
            ...
            break;
        case DBTableFieldInt:
            intVal = dbField->IntNoData();
            break;
        case DBTableFieldFloat:
            floatVal = dbField->FloatNoData();
            break;
        case DBTableFieldDate:
            break;
    }
...
static Widget _UITableCreateRecordNameMenu(Widget widget, UITableRecord *uiRecord) {
    ...
    XtSetArg (wargs[0], XmNuserData, uiRecord);
    argNum++;
    subMenu = XmCreatePulldownMenu(widget, (char *) "UITablePullDownMenu", wargs, argNum);
...
UITableRecord::UITableRecord(Widget leftWidget, Widget rightWidget, DBInt numWidth, DBInt nameWidth,
                             DBObjectLIST<UITableField> *uiFields, DBInt cellHeight) {
    Pixel foreground, background;
    UITableField *uiField;
    UITable *uiTable;
...
static void _UITableVerScrollBarCBK(Widget widget, UITable *tableCLS,
                                    XmScrollBarCallbackStruct *callData) { tableCLS->Draw(callData->value); }
...
static void _UITableLoadSelectionCBK(Widget widget, UITable *uiTable, XmAnyCallbackStruct *callData) {
    ...
    if (_UITableSelectionFieldSelect == (Widget) NULL)
        _UITableSelectionFieldSelect = UISelectionCreate((char *) "Table Field Selection");
    if (_UITableSelectionFileSelect == NULL)
        _UITableSelectionFileSelect = UIFileSelectionCreate((char *) "Selection File", NULL, (char *) "*.*",
                                                            XmFILE_REGULAR);
    if ((selection = UISelectObject(_UITableSelectionFieldSelect, (DBObjectLIST<DBObject> *) (table->Fields()),
                                    DBTableFieldIsCategory)) == (char *) NULL)
        return;
...
UITable::UITable(DBObjData *dbData, DBObjTable *table) : DBObjectLIST<UITableRecord>(table->Name(), sizeof(UITable)) {
    ...
    invertPixmap = invertPixmap == (Pixmap) NULL ?
                   XmGetPixmap(XtScreen(UITopLevel()), (char *) "GHAASinvert.bmp", foreground, background)
                                                 : invertPixmap;
    setIdlePixmap = setIdlePixmap == (Pixmap) NULL ?
                    XmGetPixmap(XtScreen(UITopLevel()), (char *) "GHAASsetIdle.bmp", foreground, background)
                                                   : setIdlePixmap;
    ...
    NameButtonWGT = XtVaCreateManagedWidget("UITableRecordNameButton", xmCascadeButtonWidgetClass, recordMenuBar,
                                            ...
                                            XmNshadowThickness, 1,
                                            XmNsubMenuId,
                                            _UITableCreateHeaderMenu(recordMenuBar, (UITableField *) NULL),
                                            XmNforeground, background,
                                            XmNbackground, foreground,
                                            NULL);
    XmStringFree(string);
    ...
    for (field = fields->First(); field != (DBObjTableField *) NULL; field = fields->Next()) {
        if ((DBTableFieldIsVisible(field) == false) ||
            ((field->Flags() & DBObjectFlagIdle) == DBObjectFlagIdle))
            continue;
        AddField(field);
    }
    StartRowVAR = 0;
...
void UITable::Resize(int height) {
    ...
    if (newRowNum > 0)
        for (row = 0; row < newRowNum; ++row) {
            Add(new UITableRecord(LeftRowColWGT, RightRowColWGT, RecordIDWidthVAR, NameWidthVAR, FieldsPTR,
                                  CellHeightVAR));
...
void UITable::Draw(DBInt startRow) {
    ...
    for (record = TablePTR->First(StartRow());
         (row < ItemNum()) && (record != (DBObjRecord *) NULL); record = TablePTR->Next())
        Draw(row++, record);
}
