...
public:
    ...
    void Button(Widget menu, int *userData) {
        ...
        button = XtVaCreateManagedWidget("UIDataPropProjectionButton", xmPushButtonGadgetClass, menu,
                                         ...
                                         NULL);
        XmStringFree(string);
        XtAddCallback(button, XmNactivateCallback, (XtCallbackProc) _UIDataPropProjectionButtonCBK,
                      (void *) ((char *) NULL + ProjectionVAR));
    }
};
...
public:
    UIDataPropertiesGeo(Widget widget) : UIDataProperties(widget, (char *) "Geographic") {
        ...
        XtAddCallback(PrecisionScaleWGT, XmNvalueChangedCallback, (XtCallbackProc) UIDataPropPrecisionCBK,
                      (void *) NULL);
    ...
    int IsChanged(DBObjData *data) {
        int value;
        XmScaleGetValue(PrecisionScaleWGT, &value);
        if (data->Precision() != value) return (true);
        XmScaleGetValue(MaxScaleScaleWGT, &value);
        if (data->MaxScale() != value) return (true);
        XmScaleGetValue(MinScaleScaleWGT, &value);
        if (data->MinScale() != value) return (true);
...
private:
    ...
    DBUnsigned FlagVAR;
public:
    UIDataPropContinuousColor(char *name, DBUnsigned flag) {
        NameSTR = name;
        FlagVAR = flag;
    }
    ...
    void MakeButton(Widget widget, DBUnsigned *userData) {
        ...
        button = XtVaCreateManagedWidget("UIDataPropContinuousColorMenuButton", xmPushButtonGadgetClass, widget,
                                         ...
                                         NULL);
        XmStringFree(string);
        XtAddCallback(button, XmNactivateCallback, (XtCallbackProc) _UIDataPropContinuousColorCBK,
                      (void *) ((char *) NULL + FlagVAR));
    }
...
public:
    ...
    int IsChanged(DBObjData *data) { return (ColorVAR != (data->Flags() & DBDataFlagDispModeContShadeSets)); }
...
DBInt UIDataPropertiesForm(DBObjData *data) {
    ...
    if (dShell == NULL) {
        ...
        if (geoForm != (Widget) NULL) XtUnmanageChild(geoForm);
    }
    ...
    switch (data->Type()) {
        ...
        case DBTypeVectorPolygon:
            currentForm = vectorProp->Load(data);
            break;
        case DBTypeGridContinuous:
            currentForm = ContGridProp->Load(data);
            break;
        case DBTypeNetwork:
            currentForm = networkProp->Load(data);
            break;
    }
    ...
    while (UILoop()) {
        ...
        switch (data->Type()) {
            case DBTypeGridContinuous:
                if (ContGridProp->IsChanged(data)) changed = true;
                break;
            default:
                break;
        }
