...
private:
    ...
    DBObjRecord *OrigREC;
public:
    UISymbol(DBObjRecord *record, DBObjTableField *symbolField, DBObjTableField *foreField, DBObjTableField *backField,
             DBObjTableField *styleField)
            : DBObjRecord(*record) {
...
static void _UISymbolLoadNamesCBK(Widget widget, Widget list, XmAnyCallbackStruct *callData) {
    ...
    while (fgets(buffer, sizeof(buffer), file) != NULL) {
        ...
        for (i = 0; i < symNum; ++i)
            if (editSymbols[i]->SymbolID() == symbol) {
                ...
                if ((token = strtok((char *) NULL, delimit)) != (char *) NULL) if (sscanf(token, "%d", &color) ==
                                                                                   1)
                    editSymbols[i]->Foreground(color);
                if ((token = strtok((char *) NULL, delimit)) != (char *) NULL) if (sscanf(token, "%d", &color) ==
                                                                                   1)
                    editSymbols[i]->Background(color);
    ...
    if (XmListGetSelectedPos(list, &selectPos, &selectCount)) {
        XmTextFieldSetString(XtNameToWidget(XtParent(XtParent(XtParent(list))), UISymTextFieldName),
                             editSymbols[selectPos[0]]->Name());
...
int UISymbolEdit(DBObjTable *symbols, int symbolType) {
    ...
    if (dShell == NULL) {
        ...
        gc = XCreateGC(XtDisplay(UITopLevel()), XtWindow(UITopLevel()), GCForeground | GCBackground | GCFillStyle,
                       &xgcv);
    ...
    switch (symbolType) {
        case UISymbolMarker:
            XtManageChild(XtNameToWidget(dShell, UISymMarkerMenuName));
            break;
        case UISymbolLine:
            XtManageChild(XtNameToWidget(dShell, UISymLineMenuName));
            break;
        case UISymbolShade:
            XtManageChild(XtNameToWidget(dShell, UISymShadeMenuName));
            break;
    }
    ...
    switch (symbolType) {
        case UISymbolMarker:
            XtUnmanageChild(XtNameToWidget(dShell, UISymMarkerMenuName));
            break;
        case UISymbolLine:
            XtUnmanageChild(XtNameToWidget(dShell, UISymLineMenuName));
            break;
        case UISymbolShade:
            XtUnmanageChild(XtNameToWidget(dShell, UISymShadeMenuName));
            break;
    }
