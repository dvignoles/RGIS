...
UIMenuItem::UIMenuItem(char *label, DBUnsigned visible, DBUnsigned sensitive,
                       void (*function)(Widget, void *, XmAnyCallbackStruct *), char *helpFile) {
...
static void _UIMenuButtonCBK(Widget widget, XtPointer *menuData, XmAnyCallbackStruct *callData) {
    ...
    switch (callData->event->xbutton.button) {
        default:
            menuBut->CallMenu(widget, menuData, callData);
            break;
        case 3:
            menuBut->CallHelp();
            break;
...
void UIMenuItem::Sensitive(unsigned groups, int sensitive) {
    ...
    switch (Type) {
        ...
        default:
            if (sensitive) { if ((SensitiveVAR & groups) == SensitiveVAR) XtSetSensitive(Button, true); }
            else { if ((SensitiveVAR & groups) == groups) XtSetSensitive(Button, false); }
            break;
    }
}
...
void UIMenuItem::Visible(unsigned groups, int visible) {
    ...
    switch (Type) {
        ...
        default:
            if (visible) { if ((VisibleVAR & groups) == VisibleVAR) XtManageChild(Button); }
            else { if ((VisibleVAR & groups) == groups) XtUnmanageChild(Button); }
            break;
    }
}

void UIMenuItem::CallMenu(Widget widget, void *data,
                          XmAnyCallbackStruct *callData) const { (*((XtCallbackProc) Action))(widget, data, callData); }
...
void UIMenuDisplayHelp(char *helpFile) {
    if (helpFile == (char *) NULL) UIMessage((char *) "Sorry, No Help! You are on your own!");
        ...
        sprintf(cmndString,"%s %s/html/rgis/%s &", getenv("GHAAS_BROWSER"), getenv ("GHAAS_DIR"), helpFile);
        system(cmndString);
    }
}
