...
void UI2DView::DrawVectorAnnotation(DBVectorIF *vectorIF, DBObjRecord *record, GC gc) {
    ...
    if ((dbData->Flags() & DBDataFlagDispModeVecAnnotSelect) == DBDataFlagDispModeVecAnnotSelect) {
        flags = record->Flags();
    ...
    XmStringFree(xmString);
    if ((dbData->Flags() & DBDataFlagDispModeVecAnnotSelect) == DBDataFlagDispModeVecAnnotSelect)
        record->Flags(DBObjectFlagSelected, (flags & DBObjectFlagSelected) == DBObjectFlagSelected);
}
...
static DBInt _UI2DViewContinuousSingleShadeFunc(DBGridIF *gridIF, DBCoordinate coord) {
    ...
    retPixel = (DBInt) ((DBFloat) (UIColorNum(_UI2DViewContPosShades)) * (value - _UI2DViewContMinValue) /
                        (_UI2DViewContMaxValue - _UI2DViewContMinValue));
...
static DBInt _UI2DViewContinuousDualShadeFunc(DBGridIF *gridIF, DBCoordinate coord) {
    ...
    if (value >= 0.0) {
        retPixel = _UI2DViewContMinValue < 0.0 ?
                   ...
                   (DBInt) ((DBFloat) (UIColorNum(_UI2DViewContPosShades)) * (value - _UI2DViewContMinValue) /
                            (_UI2DViewContMaxValue - _UI2DViewContMinValue));
        if (retPixel >= UIColorNum(_UI2DViewContPosShades)) retPixel -= 1;
        return (UIColor(_UI2DViewContPosShades, retPixel));
    }
        ...
        retPixel = _UI2DViewContMaxValue > 0.0 ?
                   (DBInt) floor((DBFloat) (UIColorNum(_UI2DViewContNegShades)) * (0.0 - value) /
                                 (0.0 - _UI2DViewContMinValue)) :
                   (DBInt) floor((DBFloat) (UIColorNum(_UI2DViewContNegShades)) * (_UI2DViewContMaxValue - value) /
                                 (_UI2DViewContMaxValue - _UI2DViewContMinValue));
...
void UI2DView::DrawGrid(DBObjData *data, GC gc) {
    ...
    if (((ViewEXT.UpperRight.X - ViewEXT.LowerLeft.X) / gridIF->CellWidth()) > (Image->width >> 0x01) ||
        ...
        endPos.x = Image->width;
        endPos.y = Image->height;
...
void UI2DView::DrawNetworkCell(DBNetworkIF *netIF, DBObjRecord *cellRec, DBUnsigned displayColor,
                               DBUnsigned displayMode, GC gc) {
    DBInt basinID;
    DBCoordinate coord[2];
    ...
    switch (displayColor) {
        ...
        case DBDataFlagDispModeNetColorSymbol:
            xgcv.foreground = (cellRec->Flags() & DBObjectFlagSelected) == DBObjectFlagSelected ?
                              UIColor(UIColorStandard,
                                      basinID != DBFault ? netIF->ItemBackground(netIF->Basin(cellRec)) : 1) :
                              UIColor(UIColorStandard,
                                      basinID != DBFault ? netIF->ItemForeground(netIF->Basin(cellRec)) : 2);
            break;
    }
