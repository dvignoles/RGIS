...
static void _UIXYGraphShellDeleteCBK(Widget widget, UIXYGraphShell *xyGraph,
                                     XmAnyCallbackStruct *callData) { delete xyGraph; }

static void _UIXYGraphShellSetDataCBK(Widget widget, UIXYGraphShell *xyGraph,
                                      XmAnyCallbackStruct *callData) { xyGraph->SetData(); }

static void _UIXYGraphShellConfigureCBK(Widget widget, UIXYGraphShell *xyGraph,
                                        XmAnyCallbackStruct *callData) { xyGraph->Configure(); }

static void _UIXYGraphShellSetListCBK(Widget widget, UIXYGraphShell *xyGraph,
                                      XmListCallbackStruct *callData) { xyGraph->SetList(callData->item_position); }

static void _UIXYGraphShellSetSeriesCBK(Widget widget, UIXYGraphShell *xyGraph,
                                        XmAnyCallbackStruct *callData) { xyGraph->SetSeries(); }

static void _UIXYGraphShellAddSeriesCBK(Widget widget, UIXYGraphShell *xyGraph,
                                        XmAnyCallbackStruct *callData) { xyGraph->AddSeries(); }

static void _UIXYGraphShellRemoveSeriesCBK(Widget widget, UIXYGraphShell *xyGraph,
                                           XmAnyCallbackStruct *callData) { xyGraph->RemoveSeries(); }
...
UIXYGraphShell::UIXYGraphShell(DBObjData *dbData) : DBObject(UIXYGraphShellStr, sizeof(UIXYGraphShell)) {
    ...
    argNum = 0;
    XtSetArg (wargs[argNum], XmNtopAttachment, XmATTACH_OPPOSITE_WIDGET);
    ++argNum;
    XtSetArg (wargs[argNum], XmNtopWidget, XAxisFieldWGT);
    ++argNum;
    XtSetArg (wargs[argNum], XmNleftAttachment, XmATTACH_FORM);
    ++argNum;
    XtSetArg (wargs[argNum], XmNleftOffset, 5);
    ++argNum;
    XtSetArg (wargs[argNum], XmNrightAttachment, XmATTACH_WIDGET);
    ++argNum;
    XtSetArg (wargs[argNum], XmNrightWidget, subForm);
    ++argNum;
    XtSetArg (wargs[argNum], XmNrightOffset, 10);
    ++argNum;
    XtSetArg (wargs[argNum], XmNbottomAttachment, XmATTACH_OPPOSITE_WIDGET);
    ++argNum;
    XtSetArg (wargs[argNum], XmNbottomWidget, subForm);
    ++argNum;
    XtSetArg (wargs[argNum], XmNshadowThickness, 2);
    ++argNum;
    XtSetArg (wargs[argNum], XmNselectionPolicy, XmSINGLE_SELECT);
    ++argNum;
    ...
    ++argNum;
    XtSetArg (wargs[argNum], XmNtopOffset, 10);
    ++argNum;
    XtSetArg (wargs[argNum], XmNleftAttachment, XmATTACH_FORM);
    ++argNum;
    XtSetArg (wargs[argNum], XmNleftOffset, 10);
    ++argNum;
    XtSetArg (wargs[argNum], XmNrightAttachment, XmATTACH_FORM);
    ++argNum;
    XtSetArg (wargs[argNum], XmNrightOffset, 10);
    ++argNum;
    XtSetArg (wargs[argNum], XmNbottomAttachment, XmATTACH_WIDGET);
    ++argNum;
    XtSetArg (wargs[argNum], XmNbottomWidget, form);
    ++argNum;
    XtSetArg (wargs[argNum], XmNbottomOffset, 5);
    ++argNum;
...
void UIXYGraphShell::Configure(DBObjRecord *record) {
    ...
    for (i = 0; i < YFieldsPTR->ItemNum(); ++i) {
        ...
        Graph->Add(new UIDataSeries(field->Name(), RelDataPTR->Table(DBrNItems), JoinFLD, XAxisFLD, field,
                                    RelFLD->String(record), i + 1));
    }
    Graph->Draw(true);
}
