...
static void _UI2DViewScaleValueChangedCBK(Widget widget, UI2DView *view, XmScaleCallbackStruct *callData) {
    ...
    viewEXT.LowerLeft.X = center.X - size.X / 2.0;
    viewEXT.LowerLeft.Y = center.Y - size.Y / 2.0;
    extent.LowerLeft = viewEXT.LowerLeft;
    viewEXT.UpperRight.X = center.X + size.X / 2.0;
    viewEXT.UpperRight.Y = center.Y + size.Y / 2.0;
...
static void _UI2DViewZoomToggleValueCBK(Widget widget, UI2DView *view, XmToggleButtonCallbackStruct *callData) {
    if (callData->set)
        view->InputMode(ZOOM_MODE);
}

static void _UI2DViewPaneToggleValueCBK(Widget widget, UI2DView *view, XmToggleButtonCallbackStruct *callData) {
    if (callData->set)
        view->InputMode(PANE_MODE);
}

static void _UI2DViewUserToggleValueCBK(Widget widget, UI2DView *view, XmToggleButtonCallbackStruct *callData) {
    if (callData->set)
        view->InputMode(USER_MODE);
}

static void _UI2DViewMeshOptionCBK(Widget widget, UI2DView *view,
                                   XmToggleButtonCallbackStruct *callData) { if (callData->set) view->DrawMesh(); else view->Draw(); }

static void _UI2DViewPointerEHR(Widget widget, UI2DView *view, XEvent *event, Boolean *boolean) {
    switch (view->InputMode()) {
        case ZOOM_MODE:
            _UI2DViewZoom(widget, event, view);
            break;
        case PANE_MODE:
            _UI2DViewPane(widget, event, view);
            break;
        case USER_MODE:
            if (_UI2DViewUserFunction != (UI2DViewUserFunction) NULL)
                (*_UI2DViewUserFunction)(_UI2DViewUserData, view, event);
            break;
        default:
            break;
    }
...
static void _UI2DViewDeleteCBK(Widget widget, UI2DView *view, XmAnyCallbackStruct *callData) { delete view; }
