...
UIDataSeries::UIDataSeries(char *name, DBCoordinate *coords, DBInt obsNum, DBInt symbol) : DBObject(name,
                                                                                                    sizeof(UIDataSeries)) {
...
UIDataSeries::UIDataSeries(char *name, DBObservation *obs, DBInt obsNum, DBInt symbol) : DBObject(name,
                                                                                                  sizeof(UIDataSeries)) {
...
UIDataSeries::UIDataSeries(char *name, DBObjTable *table, DBObjTableField *idFLD, DBObjTableField *xAxisFLD,
                           DBObjTableField *yAxisFLD, char *idString, DBInt symbol) : DBObject(name,
                                                                                               sizeof(UIDataSeries)) {
...
static void _UIXYGraphScaleChangedCBK(Widget widget, UIXYGraph *graph, XmScaleCallbackStruct *callData) {
    graph->SetViewPercent(callData->value);
...
static void _UIXYGraphScrollBarChangedCBK(Widget widget, UIXYGraph *graph, XmScaleCallbackStruct *callData) {
    graph->SetViewOffset(callData->value);
    graph->DrawSeries(true);
}
...
static void _UIXYGraphDrawingExposeCBK(Widget widget, UIXYGraph *graph,
                                       XmDrawingAreaCallbackStruct *callData) { graph->DrawSeries(false); }

static void _UIXYyAxisExposeCBK(Widget widget, UIXYGraph *graph,
                                XmDrawingAreaCallbackStruct *callData) { graph->DrawYAxis(false); }
...
static DBDate _UITimeGraphCalculateStep(DBDate *min, DBDate *max, DBInt stepNum, char *format) {
    ...
    if (step.Year() > 0) {
        ...
        min->Set(min->Year(), min->Month(), min->Day(),
                 ((DBInt) floor((double) min->Hour() / (double) step.Hour())) * step.Hour());
        max->Set(max->Year(), max->Month(), max->Day(),
                 ((DBInt) ceil((double) max->Hour() / (double) step.Hour())) * step.Hour());
    }
    else {
        min->Set(min->Year(), min->Month(), min->Day(), min->Hour(),
                 ((DBInt) floor((double) min->Minute() / (double) step.Minute())) * step.Minute());
        max->Set(max->Year(), max->Month(), max->Day(), max->Hour(),
                 ((DBInt) ceil((double) max->Minute() / (double) step.Minute())) * step.Minute());
    }
    return (step);
}
...
void UIXYGraph::SetRange() {
    ...
    if (series->Type() == UIDataXYSeries) {
        ...
        XYStepVAR.X = _UIXYGraphCalculateStep(&XYRangeVAR.LowerLeft.X, &XYRangeVAR.UpperRight.X, width / 100,
                                              XFormatSTR);
        XYStepVAR.Y = _UIXYGraphCalculateStep(&XYRangeVAR.LowerLeft.Y, &XYRangeVAR.UpperRight.Y, height / 40,
                                              YFormatSTR);
    }
    else {
        for (; series != (UIDataSeries *) NULL; series = Next())
            ObsRangeVAR.Expand(series->ObsRange());
        ObsStepVAR.Date = _UITimeGraphCalculateStep(&ObsRangeVAR.Min.Date, &ObsRangeVAR.Max.Date, width / 100,
                                                    XFormatSTR);
        ObsStepVAR.Value = _UIXYGraphCalculateStep(&ObsRangeVAR.Min.Value, &ObsRangeVAR.Max.Value, height / 40,
                                                   YFormatSTR);
    }
}
...
void UIXYGraph::DrawSeries(int clear) {
    ...
    if (series->Type() == UIDataXYSeries) {
        DBCoordinate coord;
        for (stepNum = 0;
             stepNum <= (DBInt) ((XYRangeVAR.UpperRight.X - XYRangeVAR.LowerLeft.X) / XYStepVAR.X); ++stepNum) {
            x = yAxisWidth + (DBInt) ((stepNum * XYStepVAR.X) * (width * 100 / ViewPercentVAR) /
            ...
            XmStringDraw(XtDisplay(DrawingAreaWGT), XtWindow(DrawingAreaWGT), UISmallFontList(),
                         string, Gc, x - strWidth, yAxisHeight + 5, strWidth, XmALIGNMENT_END,
        ...
        for (series = First(); series != (UIDataSeries *) NULL; series = Next()) {
            ...
            x0 = yAxisWidth + (DBInt) ((coord.X - XYRangeVAR.LowerLeft.X) * (width * 100 / ViewPercentVAR) /
                                       (XYRangeVAR.UpperRight.X - XYRangeVAR.LowerLeft.X)) - 1;
            x0 -= width * ViewOffsetVAR / ViewPercentVAR;
            y0 = yAxisHeight - (DBInt) ((coord.Y - XYRangeVAR.LowerLeft.Y) * yAxisHeight /
                                        (XYRangeVAR.UpperRight.Y - XYRangeVAR.LowerLeft.Y));
            for (i = 1; i < series->ObsNum(); ++i) {
                coord = series->XYData(i);
                if (CMmathEqualValues(coord.Y, series->NoData())) continue;
                x1 = yAxisWidth + (DBInt) ((coord.X - XYRangeVAR.LowerLeft.X) * (width * 100 / ViewPercentVAR) /
                                           (XYRangeVAR.UpperRight.X - XYRangeVAR.LowerLeft.X)) - 1;
                x1 -= width * ViewOffsetVAR / ViewPercentVAR;
                y1 = yAxisHeight - (DBInt) ((coord.Y - XYRangeVAR.LowerLeft.Y) * yAxisHeight /
                                            (XYRangeVAR.UpperRight.Y - XYRangeVAR.LowerLeft.Y));
        ...
        DBObservation obs;
        for (date = ObsRangeVAR.Min.Date;
             date.MinutesAD() <= ObsRangeVAR.Max.Date.MinutesAD(); date = date + ObsStepVAR.Date) {
            x = yAxisWidth + (DBInt) ((width * 100 / ViewPercentVAR) *
                                      ((DBFloat) (date.MinutesAD() - ObsRangeVAR.Min.Date.MinutesAD())) /
        ...
        for (series = First(); series != (UIDataSeries *) NULL; series = Next()) {
            ...
            x0 = (DBInt) ((width * 100.0 / ViewPercentVAR) *
                          ((DBFloat) (obs.Date.MinutesAD() - ObsRangeVAR.Min.Date.MinutesAD())) /
                          (ObsRangeVAR.Max.Date.MinutesAD() - ObsRangeVAR.Min.Date.MinutesAD()));
            x0 -= (DBInt) (width * ((double) ViewOffsetVAR / (double) ViewPercentVAR));
            x0 += yAxisWidth;
            y0 = yAxisHeight - (DBInt) ((obs.Value - ObsRangeVAR.Min.Value) * yAxisHeight /
                                        (ObsRangeVAR.Max.Value - ObsRangeVAR.Min.Value));
            for (i = 1; i < series->ObsNum(); ++i) {
                obs = series->ObsData(i);
                if (CMmathEqualValues(obs.Value, series->NoData())) continue;
                x1 = (DBInt) ((width * 100 / ViewPercentVAR) *
                              ((DBFloat) (obs.Date.MinutesAD() - ObsRangeVAR.Min.Date.MinutesAD())) /
                              (ObsRangeVAR.Max.Date.MinutesAD() - ObsRangeVAR.Min.Date.MinutesAD())) - 1;
                x1 -= width * ViewOffsetVAR / ViewPercentVAR;
                x1 += yAxisWidth;
                y1 = yAxisHeight - (DBInt) ((obs.Value - ObsRangeVAR.Min.Value) * yAxisHeight /
                                            (ObsRangeVAR.Max.Value - ObsRangeVAR.Min.Value));
                XDrawLine(XtDisplay(DrawingAreaWGT), XtWindow(DrawingAreaWGT), Gc, x0, y0, x1, y1);
                x0 = x1;
                y0 = y1;
...
void UIXYGraph::DrawYAxis(int clear) {
    ...
    if (series->Type() == UIDataXYSeries)
        for (stepNum = 0;
             stepNum < (DBInt) ((XYRangeVAR.UpperRight.Y - XYRangeVAR.LowerLeft.Y) / XYStepVAR.Y); ++stepNum) {
            y = height -
            ...
            XmStringDraw(XtDisplay(YAxisWGT), XtWindow(YAxisWGT), UISmallFontList(),
                         string, Gc, width - strWidth - 5, y - strHeight, strWidth, XmALIGNMENT_END,
                         XmSTRING_DIRECTION_L_TO_R, NULL);
            XmStringFree(string);
        }
    else
        for (stepNum = 0;
             stepNum < (DBInt) ((ObsRangeVAR.Max.Value - ObsRangeVAR.Min.Value) / ObsStepVAR.Value); ++stepNum) {
            y = height -
...
void UIXYGraph::Initialize(Widget parent, char *title, Arg wargs[], int argNum, XtCallbackProc callbackProc,
                           XtPointer callData) {
    ...
    ScrollBarWGT = XtVaCreateManagedWidget("UIXYGraphScrollBar", xmScrollBarWidgetClass, scrolledWindow,
                                           ...
                                           XmNsliderSize, 100,
                                           NULL);
    XtAddCallback(ScrollBarWGT, XmNvalueChangedCallback, (XtCallbackProc) _UIXYGraphScrollBarChangedCBK,
                  (XtPointer) this);
...
UIXYGraph::UIXYGraph() : DBObjectLIST<UIDataSeries>("UIXYGraph",
                                                    sizeof(UIXYGraph)) { MainWGT = ScrollBarWGT = ScaleWGT = DrawingAreaWGT = (Widget) NULL; }
...
void UIXYGraph::Add(UIDataSeries *series) {
    ...
    if ((firstSeries != (UIDataSeries *) NULL) && (firstSeries->Type() != series->Type())) {
        CMmsgPrint(CMmsgAppError, "Invalid Data Series in: %s %d", __FILE__, __LINE__);
        return;
    }
