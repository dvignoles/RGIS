...
void UI2DView::SetActiveExtent(DBRegion extent) {
    ...
    ActiveEXT.LowerLeft = lowerLeft;
    ActiveEXT.UpperRight = upperRight;
...
void UI2DView::Size() {
    ...
    Image = XCreateImage(XtDisplay(DrawingAreaW), xwa.visual, xwa.depth, ZPixmap, 0, data, xwa.width, xwa.height,
                         bitmap_pad, 0);
    if (FullRegion != NULL) XDestroyRegion(FullRegion);
    poly[0].x = poly[1].x = 0;
    poly[2].x = poly[3].x = xwa.width;
    poly[0].y = poly[3].y = 0;
    poly[1].y = poly[2].y = xwa.height;
