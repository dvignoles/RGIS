...
char *UIAuxGetLabelString(Widget widget) { return (UIAuxGetLabelString(widget, UICharSetNormal)); }

void UIAuxSetBooleanTrueCBK(Widget widget, int *boolean, XmAnyCallbackStruct *callData) { *boolean = True; }

void UIAuxSetBooleanFalseCBK(Widget widget, int *boolean, XmAnyCallbackStruct *callData) { *boolean = False; }
...
void UIAuxSetToggleCBK(Widget widget, DBInt *toggle,
                       XmToggleButtonCallbackStruct *callData) { *toggle = callData->set; }
...
void UIAuxSetDefaultButtonEH(Widget widget, void *data, XEvent *event, Boolean boolVal) {
    ...
    switch (event->type) {
        case EnterNotify:
            XtSetArg (wargs[0], XmNshowAsDefault, True);
            break;
        case LeaveNotify:
            XtSetArg (wargs[0], XmNshowAsDefault, False);
            break;
    }
...
void UIAuxFileSelectCBK(Widget widget, Widget text, XmAnyCallbackStruct *callData) {
    ...
    if (select == (Widget) NULL)
        select = UIFileSelectionCreate((char *) "File Selection", (char *) NULL, (char *) NULL, XmFILE_REGULAR);
