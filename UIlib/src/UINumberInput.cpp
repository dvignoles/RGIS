...
public:
    ...
    DBInt ScaleValue(DBFloat value) {
        if (ModeVAR == UINumberInputModeLogarithmic)
            return ((DBInt) ((log10(value) - log10(MinVAR)) * (MaxScaleVAR - MinScaleVAR) /
                             (log10(MaxVAR) - log10(MinVAR))));
        else
    ...
    DBFloat Value(DBInt scale) {
        if (ModeVAR == UINumberInputModeLogarithmic)
            return (pow(10.0, log10(MinVAR) +
                              (log10(MaxVAR) - log10(MinVAR)) * (DBFloat) scale / (MaxScaleVAR - MinScaleVAR)));
        else
    ...
    char *ValueString(DBInt scaleValue) { return (ValueString(Value(scaleValue))); }
};
...
Widget UINumberInputCreate(Widget parent, char *labelText, Arg *wargs, DBInt argNum, DBFloat min, DBFloat max,
                           DBFloat value, DBInt mode, char *format) {
...
Widget UINumberInputGetScale(Widget form) { return (XtNameToWidget(form, UINumberInputScale)); }
