...
static void doHelp(bool extended, const char *progName) {
    if (extended) {
...
int main(int argc, char *argv[]) {
    ...
    if (CMargTest(argv[1], "-V", "--verbose")) {
        verbose = true;
        argNum = CMargShiftLeft(1, argv, argNum);
    }
    ...
    for (argPos = 1; argPos < argNum;) {
        ...
        if (CMargTest(argv[argPos], "-b", "--table")) {
            ...
            if (verbose)
                CMmsgPrint(CMmsgDebug, "relateTableName: '%s'joinTableName: '%s'", relateTableName, joinTableName);
    ...
    if ((argNum > 1) && (strcmp(argv[1], "-") != 0)) relateData->Read(argv[1]);
        ...
        CMmsgPrint(CMmsgInfo, "WARN: reading from STDIN!");
    ...
    if ((relateData == joinData) && (relateTable == joinTable)) {
        CMmsgPrint(CMmsgUsrError, "Tables are identical!");
        delete relateData;
        delete joinData;
        return (DBFault);
    }
    ...
    for (int i = 0; i < numGrps; i++) {
        if ((field = relateTable->Field(groups[i]->relate)) == (DBObjTableField *) NULL) {
            CMmsgPrint(CMmsgUsrError, "Invalid Relate Table group name: %s", groups[i]->relate);
            return (CMfailed);
        }
        groups[i]->relateFLD = field;
        if ((field = joinTable->Field(groups[i]->join)) == (DBObjTableField *) NULL) {
            CMmsgPrint(CMmsgUsrError, "Invalid Join Table group name: %s", groups[i]->join);
            return (CMfailed);
        }
        groups[i]->joinFLD = field;
        if (verbose)
            CMmsgPrint(CMmsgUsrError, "Added Group #%d: join: '%s' <-> relate: '%s'", i, groups[i]->join,
                       groups[i]->relate);
    }
    if (verbose)
        CMmsgPrint(CMmsgDebug, "relateItemNum: %d joinItemNum: %d", relateTable->ItemNum(), joinTable->ItemNum());
...
//	for (int joinRecID = joinTable->ItemNum() - 1;joinRecID != -1;++joinRecID) // use this line when multiple matches add LAST found
    {
        ...
        for (int relateRecID = 0; relateRecID < relateTable->ItemNum(); ++relateRecID) {
            ...
            int i = 0;
            while (i < numGrps &&
                   DBTableFieldMatch(groups[i]->joinFLD, joinRecord, groups[i]->relateFLD, relateRecord))
                i++;
            if (i == numGrps) {
                i = 0;
                while (i < numFlds) {
