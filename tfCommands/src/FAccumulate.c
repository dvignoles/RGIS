    ...
    while ((line = FGetLine(line, &bSize, stdin)) != (char *) NULL) {
        ...
        if (count) {
            ...
            for (a = 0; a < (int) strlen(fieldBuffer); ++a) {
                if (((fieldBuffer[a] < '0') && (fieldBuffer[a] != '.')) ||
                    ((fieldBuffer[a] > '9') && (fieldBuffer[a] != '.'))) {
                    /*String*/
                    Type = 3;
                }
                else {
                ...
                if (Type == 3) {
                    CMmsgPrint(CMmsgUsrError,
                               "String (or two decimal points) found where Integer or Float was expected in main()!\nfield: \'%s\', row#: %d, col#: %d.",
