...
/*****globals***/
FILE *dataFile = NULL;
/*this will point to the actual data file*/
...
void setInputFiles(int argc, char *argv[]) {
    ...
    for (j = 1; j < argc; j++) {
        if (access(argv[j], R_OK) == 0) if (!dataTypesFile || j != i + 1)
...
void reg(const int xFieldNum, const int yFieldNum, const int wFieldNum, const fData *theData) {
    ...
    if (sumW == 0) {
        CMmsgPrint(CMmsgUsrError, "I could not read the \"weight field\" as numeric values.");
        return;
    }
    if (sumX == 0)
