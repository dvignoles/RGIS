...
void showUsage() {
    ...
    CMmsgPrint(CMmsgInfo,
               "\t-p,--pct all <columnNumber>\treturns a the original table with a new column of quantiles for <columnNumber>");
    ...
    CMmsgPrint(CMmsgInfo,
               "  -output the minimum value of column 0 and a set of quantiles for column 5 all out of \"test.txt\"");
    CMmsgPrint(CMmsgInfo, "%% FStat --num --pct all 5 < test.txt");
    CMmsgPrint(CMmsgInfo,
               "  -this will output the number of lines in \"test.txt\", and then output the table with\n   a new column of quantiles for every entry in column 5.");
}
...
int findArg(char *shortName, char *longName, int argc, char *argv[]) {
    int i = 1;
    while (i < argc) {
        if (argv[i] && (!strcmp(argv[i], shortName) || !strcmp(argv[i], longName))) return i;
        i++;
    }
...
void setInputFiles(int argc, char *argv[]) {
    ...
    for (j = 1; j < argc; j++) {
        if (access(argv[j], R_OK) == 0) if (!dataTypesFile || j != i + 1) dataFile = fopen(argv[j], "rb");
    }
...
int main(int argc, char *argv[]) {
    ...
    for (argPos = 1; argPos < argNum;) {
        ...
        if (CMargTest (argv[argPos], "-p", "--pct")) {
            if ((argNum = CMargShiftLeft(argPos, argv, argNum)) <= argPos) if (argPos >= argNum) {
...
void pct(fData *theData, float *pctMark, const int colNumber) {
    ...
    else /*--pct "q1,q2,q3,...*/
    {
        ...
        while (j++ < pctMark[0]) {
            ...
            for (i = 0;
                 (((float) i / ((float) theData->numRows)) * 100.0) < currentNum && (i + 1) < theData->numRows; i++);
...
void dev(fData *theData, const int colNumber) { /*note this is NON-BIASED standard deviation*/
    float avg = 0, sum = 0;
    int i = 0;
