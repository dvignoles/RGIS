...
private:
    ...
    char Level0Password[4];
    /* level 0 password */
    char Level1Password[4];
    /* level 1 password */
    char Level2Password[4];
    /* level 2 password */
    char Level3Password[4];
    /* level 3 password */
    short PrintWidth;
    /* report output width */
    short EmptyFlag;
    /* directory record empty flag */
    int NumberRecords;
    /* number of records */
    short Filler1;
    /* not used */
...
private:
    ...
    short KeyTypeVAR;
    /* key type */
    short ItemPositionVAR;
    /* item position */
    short ProtectionLevelVAR;
    /* item protection level */
    short Filler1VAR;
    /* ? */
    short OutputWidthVAR;
    /* item output width */
    short NumberDecimalsVAR;
    /* number of decimal places */
    short ItemTypeVAR;
    /* item type */
    short ReadOnlyFlagVAR;
    /* read only flag */
    short KeyLevelVAR;
    /* key level */
    short IndexNumberVAR;
    /* item index number */
    short OccuranceCountVAR;
    /* number of occurrences */
    short Filler2VAR;
    /* ? */
    char AlternateNameSTR[16];
    /* item alternate name */
    char Filler3STR[16];
    /* ? */
    short ConcordLevelVAR;
    /* concordance level */
    short AlternateRuleVAR;
    /* item naming rule */
    char Filler4STR[32];
    /* ? */
    short ConcordCaseVAR;
    /* concordance case */
    short ConcordNumbersVAR;
    /* concordance numbers */
    short ItemNumberVAR;
    /* logical item number */
    char Filler5STR[28];
...
public:
    ...
    DBObjTableField *MakeField(FILE *file, DBInt swap) {
        ...
        strncpy(name, ItemNameSTR, sizeof(ItemNameSTR));
        name[sizeof(ItemNameSTR)] = '\0';
...
char *DBInfoFileName(const char *workSpace, const char *infoName) {
    ...
    while (arcDirRecord.Read(inFile, swap) == DBSuccess)
        if ((strcmp(arcDirRecord.GetInternalName(), infoName) == 0) &&
            (arcDirRecord.GetEmptyFlag() == false)) {
            notFound = false;
            break;
        }
    fclose(inFile);
...
static void _DBInfoGetField(DBObjTableField *field, DBObjRecord *record, ARCNitRecord *arcNitRecord, void *infoBuffer,
                            DBInt swap) {
            ...
            switch (arcNitRecord->ItemType()) {
                ...
                case INFO_BINARY_TYPE:
                    switch (arcNitRecord->ItemWidth()) {
                        ...
                        default:
                            CMmsgPrint(CMmsgAppError, "Invalid Item Width in: %s %d", __FILE__, __LINE__);
                            break;
                    }
                    break;
                default:
                    CMmsgPrint(CMmsgAppError, "Invalid Info Field in: %s %d", __FILE__, __LINE__);
                    break;
            }
...
DBInt DBInfoGetTable(DBObjTable *table, const char *infoName) {
    ...
    for (arcNitRecord = (ARCNitRecord *) malloc(sizeof(ARCNitRecord));
        ...
        if (arcNitRecord == (ARCNitRecord *) NULL) {
            CMmsgPrint(CMmsgSysError, "Memory (Re)allocation Error in: %s %d", __FILE__, __LINE__);
            return (DBFault);
        }
    ...
    for (i = 0; i < fieldNum; ++i) {
        strncpy(name, arcNitRecord[i].Name(), 16);
        name[16] = '\0';
    ...
    if (external) {
        ...
        sep = 0;
        for (i = strlen(name); (i >= 0) && (sep < 2); --i) if (name[i] == '/') ++sep;
        sep = 0;
        for (j = strlen(fileName); (j >= 0) && (sep < 2); --j) if (fileName[j] == '/') ++sep;
    ...
    for (record = table->First(); fread(buffer, recordLen, 1, inFile) == 1; record = table->Next()) {
        ...
        if (record == (DBObjRecord *) NULL) {
            CMmsgPrint(CMmsgAppError, "Null Record in: %s %d", __FILE__, __LINE__);
            return (DBFault);
        }
