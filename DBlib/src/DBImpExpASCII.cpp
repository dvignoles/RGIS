...
public:
    ...
    const char *Field(int fieldID) const {
        if (fieldID >= FieldNumVAR)
            return ((char *) NULL);
        else return (FieldARRAY[fieldID]);
    }
...
public:
    _DBImportASCIIHeader(const char *buffer) : _DBImportASCIIRecord(buffer) {
        if ((FieldsPTR = (_DBImportASCIIHeaderField *) calloc(FieldNum(), sizeof(_DBImportASCIIHeaderField))) ==
            (_DBImportASCIIHeaderField *) NULL)
            CMmsgPrint(CMmsgSysError, "Memory Allocation Error in: %s %d", __FILE__, __LINE__);
    }
...
DBInt DBImportASCIITable(DBObjTable *table, FILE *fp) {
    ...
    if (header->Ready()) {
        while ((buffer = _DBGetLine(buffer, &bufferSize, fp)) != (char *) NULL) {
            if ((records = (_DBImportASCIIRecord **) realloc(records,
                                                             sizeof(_DBImportASCIIRecord *) * (recordNum + 1))) ==
                (_DBImportASCIIRecord **) NULL) {
                CMmsgPrint(CMmsgSysError, "Memory Allocation Error in: %s %d", __FILE__, __LINE__);
                delete header;
                ret = DBFault;
                goto Stop;
        ...
        for (fieldID = 0; fieldID < header->FieldNum(); fieldID++) {
            ...
            for (recordID = 0; recordID < recordNum; recordID++) {
                ...
                if ((header->Type(fieldID) == DBVariableInt) && CMmathIsInteger(fieldSTR)) {
                    sscanf(fieldSTR, "%d", &intVal);
                    maxInt = abs(intVal) > maxInt ? abs(intVal) : maxInt;
                    maxFloat = (double) maxInt;
                    continue;
                }
                else header->Type(fieldID, DBVariableFloat);

                if ((header->Type(fieldID) == DBVariableFloat) && (sscanf(fieldSTR, "%lg", &floatVal) == 1)) {
                    maxFloat = fabs(floatVal) > maxFloat ? fabs(floatVal) : maxFloat;
                    continue;
                }
            ...
            for (fieldID = 0; fieldID < header->FieldNum(); fieldID++) {
                ...
                switch (header->Type(fieldID)) {
                    ...
                    case DBVariableString:
                        ...
                        break;
                    case DBVariableInt:
                        if (sscanf(records[recordID]->Field(fieldID), "%d", &intVal) != 1)
                            intVal = fieldFLD->IntNoData();
                        fieldFLD->Int(recordPTR, intVal);
                        break;
                    case DBVariableFloat:
                        if (sscanf(records[recordID]->Field(fieldID), "%lf", &floatVal) != 1)
                            floatVal = fieldFLD->FloatNoData();
