...
DBObjData *DBGridToGrid(DBObjData *srcGridData) { return (DBGridToGrid(srcGridData, srcGridData->Type())); }
...
DBObjData *DBGridMerge(DBObjData *grd0Data, DBObjData *grd1Data) {
    ...
    if (((grd0Data->Type() != DBTypeGridDiscrete) && (grd0Data->Type() != DBTypeGridContinuous) &&
    ...
    switch (grd0Data->Type()) {
        case DBTypeGridContinuous:
            if ((retData = DBGridCreate(grd0Data->Name(), extent, cellSize, DBTypeGridContinuous)) ==
                (DBObjData *) NULL) {
