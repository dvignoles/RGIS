...
DBInt DBTableFieldMatch(const DBObjTableField *field0, const DBObjRecord *record0, const DBObjTableField *field1,
                        const DBObjRecord *record1) {
...
void DBObjTableField::Initialize(DBInt type, const char *format, DBUnsigned length, DBInt required) {
    ...
    switch (type) {
        ...
        case DBTableFieldInt:
            switch (length) {
                case sizeof(DBByte):
                    BytePROP.NoData(DBDefaultMissingByteVal);
                    break;
                case sizeof(DBShort):
                    ShortPROP.NoData(DBDefaultMissingIntVal);
                    break;
                case sizeof(DBInt):
                    IntPROP.NoData(DBDefaultMissingIntVal);
                    break;
            }
        ...
        case DBTableFieldFloat:
            switch (length) {
                case sizeof(DBFloat4):
                    Float4PROP.NoData(DBDefaultMissingFloatVal);
                    break;
                case sizeof(DBFloat):
                    FloatPROP.NoData(DBDefaultMissingFloatVal);
                    break;
            }
            strncpy(FormatSTR, format, DBStringLength);
            break;
...
DBDate DBObjTableField::Date(const DBObjRecord *record) const {
    ...
    switch (Type()) {
        case DBTableFieldString:
        case DBTableFieldInt:
            value.Set(String(record));
            break;
        case DBTableFieldDate:
            memcpy(&value, (char *) record->Data() + StartByte(), sizeof(DBDate));
            break;
    }
    return (value);
}
...
DBCoordinate DBObjTableField::Coordinate(const DBObjRecord *record) const {
    DBCoordinate value;
    memcpy(&value, (char *) record->Data() + StartByte(), sizeof(DBCoordinate));
    return (value);
}
...
DBRegion DBObjTableField::Region(const DBObjRecord *record) const {
    DBRegion value;
    memcpy(&value, (char *) record->Data() + StartByte(), sizeof(DBRegion));
    return (value);
}
...
void DBObjTable::ListSort(DBObjectLIST<DBObjTableField> *fields) {
    _DBObjTableSortFields = fields;
    ListSort(_DBObjTableListSort);
}
