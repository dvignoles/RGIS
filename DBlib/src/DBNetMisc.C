...
DBObjRecord *DBNetworkIF::Cell(DBPosition pos, DBObjTableField *field, DBFloat target, DBInt pRadius) const {
    DBInt i, j, pR2 = pRadius * pRadius;
    DBInt cellID;
    ...
    bestDelta = HUGE_VAL;
    for (i = -pRadius; i <= pRadius; ++i) for (j = -pRadius; j <= pRadius; ++j) {
