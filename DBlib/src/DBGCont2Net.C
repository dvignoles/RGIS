...
DBInt DBGridCont2Network(DBObjData *gridData, DBObjData *netData, bool downhill, bool build) {
    ...
    if ((zGridData != (DBObjData *) NULL) &&
        ((zGridData->Type() == DBTypeGridDiscrete) || (zGridData->Type() == DBTypeGridContinuous))) {
        zGridIF = new DBGridIF(zGridData);
        zLayerNum = zGridIF->LayerNum() + 1;
    }
    else {
        zGridIF = (DBGridIF *) NULL;
        zLayerNum = 1;
    }
                ...
                for (; zLayerID < zGridIF->LayerNum(); zLayerID++) {
                    ...
                    for (dir = 0; dir < 8; ++dir) {
                        ...
                        if (((0x01 << dir) == DBNetDirNW) || ((0x01 << dir) == DBNetDirN) ||
                            ((0x01 << dir) == DBNetDirNE))
                            row++;
                        if (((0x01 << dir) == DBNetDirSE) || ((0x01 << dir) == DBNetDirS) ||
                            ((0x01 << dir) == DBNetDirSW))
                            row--;
                        if (((0x01 << dir) == DBNetDirNE) || ((0x01 << dir) == DBNetDirE) ||
                            ((0x01 << dir) == DBNetDirSE))
                            col++;
                        if (((0x01 << dir) == DBNetDirNW) || ((0x01 << dir) == DBNetDirW) ||
                            ((0x01 << dir) == DBNetDirSW))
                            col--;
                        if (row < 0) continue;
                        if (col < 0) continue;
                        ...
                        switch (zGridData->Type()) {
                            case DBTypeGridDiscrete:
                                basinID = zGridIF->GridValue(layerRec, coord1);
                                break;
                            case DBTypeGridContinuous:
                                zGridIF->Value(layerRec, coord1, &basinID);
                                break;
                        }
                    ...
                    switch (zGridData->Type()) {
                        case DBTypeGridDiscrete:
                            basinID = zGridIF->GridValue(layerRec, coord0);
                            break;
                        case DBTypeGridContinuous:
                            zGridIF->Value(layerRec, coord0, &basinID);
                            break;
                    }
                ...
                if (gridIF->Value(layerRec, pos, &elev0)) {
                    ...
                    for (zLayerID = 0; zLayerID < zLayerNum; ++zLayerID) {
                        for (dir = 0; dir < 8; ++dir) {
                            ...
                            if ((zones[zLayerID * 9 + dir] == zones[zLayerID * 9 + 8]) &&
                                (gridIF->Value(layerRec, auxPos, &elev1))) {
