...
char *formatString(const fDataType t) {
    ...
    switch (t.type) {
        case(INT):
            sprintf(ret, "%%%dd", t.sigDigits);
            break;
        case(STRING):
            sprintf(ret, "%%s");
            break;
        case(FLOAT):
            sprintf(ret, "%%%d.%df", (t.sigDigits), (t.inSigDigits));
            break;
        default:
            sprintf(ret, "unknown");
    }
...
int verifyType(const char *data, const fDataType type) {
    ...
    if (comp.type == STRING) {
        CMmsgPrint(CMmsgUsrError, "\twrong type");
        return 0;
    }
    if (comp.type == FLOAT && type.type == INT) {
        CMmsgPrint(CMmsgUsrError, "\twrong type");
        return 0;
    }
    ...
    if (comp.type == FLOAT) {
        if (comp.sigDigits > type.sigDigits) {
            CMmsgPrint(CMmsgUsrError, "\tsig.digits too low");
            return 0;
        }
        else if (comp.inSigDigits > type.inSigDigits) {
            CMmsgPrint(CMmsgUsrError, "\tinsig.digits too low");
            return 0;
        }
        else return 1;
    }
