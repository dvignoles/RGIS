...
DBInt RGlibGridNetFilter(DBObjData *netData, DBObjData *grdData) {
    DBInt layerID, cellID, count, ret = CMfailed, kernel, kernelSize, maxProgress, dir;
        ...
        switch (inGData->Type()) {
            ...
            case DBTypeGridDiscrete: {
                ...
                for (pos.Row = 0; pos.Row < outGridIF->RowNum(); pos.Row++) {
                    ...
                    for (pos.Col = 0; pos.Col < outGridIF->ColNum(); pos.Col++)
                        if ((netIF != (DBNetworkIF *) NULL) && (netIF->Cell(pos) == (DBObjRecord *) NULL))
                            outGridIF->Value(outLayerRec, pos,
                            ...
                            outGridIF->Value(outLayerRec, pos, inGrdRec != (DBObjRecord *) NULL ? inGrdRec->RowID() :
                                                               (noDataRec != (DBObjRecord *) NULL ? noDataRec->RowID()
                                                                                                  : DBFault));
                        }
                }
            }
        ...
        if (outGData->Type() == DBTypeGridDiscrete) {
            ...
            for (inFLD = inFields->First(); inFLD != (DBObjTableField *) NULL; inFLD = inFields->Next()) {
                if ((outFLD = outTable->Field(inFLD->Name())) == (DBObjTableField *) NULL) {
                    outFLD = new DBObjTableField(*inFLD);
                    outTable->AddField(outFLD);
                }
                for (dataID = 0; dataID < inTable->ItemNum(); ++dataID) {
                    inDataRec = inTable->Item(dataID);
                    ...
                    switch (inFLD->Type()) {
                        case DBTableFieldString:
                            outFLD->String(outDataRec, inFLD->String(inDataRec));
                            break;
                        case DBTableFieldInt:
                            outFLD->Int(outDataRec, inFLD->Int(inDataRec));
                            break;
                        case DBTableFieldFloat:
                            outFLD->Float(outDataRec, inFLD->Float(inDataRec));
                            break;
                        case DBTableFieldDate:
                            outFLD->Date(outDataRec, inFLD->Date(inDataRec));
                            break;
...
DBInt RGlibGridUniformRunoff(DBObjData *gridData, DBObjData *tsData, char *grdRelate, char *tsJoin, char *tsTime,
                             char *tsVar, DBObjData *runoffData) {
    ...
    if (grdTBL->Field(RGlibArea) == (DBObjTableField *) NULL) {
        CMmsgPrint(CMmsgUsrError, "Missing Subbasin Area Field!");
        return (DBFault);
    }
    if (grdTBL->Field(grdRelate) == (DBObjTableField *) NULL) {
        CMmsgPrint(CMmsgUsrError, "Invalid Relate Field!");
        return (DBFault);
    }
    if ((tsJoinFLD = tsTBL->Field(tsJoin)) == (DBObjTableField *) NULL) {
        CMmsgPrint(CMmsgUsrError, "Invalid Join Field!");
        return (DBFault);
    }
    if ((tsTimeFLD = tsTBL->Field(tsTime)) == (DBObjTableField *) NULL) {
        CMmsgPrint(CMmsgUsrError, "Invalid Time Field!");
        return (DBFault);
    }
    if ((tsVarFLD = tsTBL->Field(tsVar)) == (DBObjTableField *) NULL) {
        CMmsgPrint(CMmsgUsrError, "Invalid Variable Field!");
        return (DBFault);
    }
                ...
                for (grdRec = grdTBL->Last(); grdRec != (DBObjRecord *) NULL; grdRec = grdTBL->Next(DBBackward)) {
                    ...
                    for (nextGrdRec = grdTBL->Item(grdNextStnFLD->Int(grdRec) - 1);
                        ...
                        if ((nextGrdRec->Flags() & DBObjectFlagLocked) == DBObjectFlagLocked) {
                            grdTmpInterFluFLD->Float(nextGrdRec,
                                                     grdTmpInterFluFLD->Float(nextGrdRec) - grdAreaFLD->Float(grdRec));
                            grdTmpValueFLD->Float(nextGrdRec,
                                                  grdTmpValueFLD->Float(nextGrdRec) - grdTmpValueFLD->Float(grdRec));
                            break;
                        }
                }
                for (grdRec = grdTBL->Last(); grdRec != (DBObjRecord *) NULL; grdRec = grdTBL->Next(DBBackward))
                    if ((grdRec->Flags() & DBObjectFlagLocked) == DBObjectFlagLocked)
                        grdTmpValueFLD->Float(grdRec, grdTmpValueFLD->Float(grdRec) * 31536.0 /
                                                      grdTmpInterFluFLD->Float(grdRec));
                    else {
    ...
    if (tsRowNum == tsTBL->ItemNum()) {
        ...
        for (grdRec = grdTBL->Last(); grdRec != (DBObjRecord *) NULL; grdRec = grdTBL->Next(DBBackward))
            if ((grdRec->Flags() & DBObjectFlagLocked) == DBObjectFlagLocked)
                grdTmpValueFLD->Float(grdRec,
                                      grdTmpValueFLD->Float(grdRec) * 31536.0 / grdTmpInterFluFLD->Float(grdRec));
            else {
                if ((nextGrdRec = grdTBL->Item(grdNextStnFLD->Int(grdRec) - 1)) != (DBObjRecord *) NULL)
                    grdTmpValueFLD->Float(grdRec, grdTmpValueFLD->Float(nextGrdRec));
                else grdTmpValueFLD->Float(grdRec, runoffIF->MissingValue());
...
DBInt RGlibGridUniformGrid(DBObjData *gridData, DBObjData *tabData, char *relateName, char *joinName, char *valueName,
                           char *dateName, DBObjData *outData) {
    DBInt first = true, tsRowNum = 0, ret;
    DBPosition pos;
    char date[DBStringLength], curDate[DBStringLength];
    DBObjTable *grdTBL = gridData->Table(DBrNItems);
    ...
    if ((relateFLD = grdTBL->Field(relateName)) == (DBObjTableField *) NULL) {
        CMmsgPrint(CMmsgAppError, "Invalid relate field [%s] in: %s %d", relateName, __FILE__, __LINE__);
        return (DBFault);
    }
    if ((joinFLD = datTBL->Field(joinName)) == (DBObjTableField *) NULL) {
        CMmsgPrint(CMmsgAppError, "Invalid join field [%s] in: %s %d", joinName, __FILE__, __LINE__);
        return (DBFault);
    }
    if ((valueFLD = datTBL->Field(valueName)) == (DBObjTableField *) NULL) {
        CMmsgPrint(CMmsgAppError, "Invalid value field [%s] in: %s %d", valueName, __FILE__, __LINE__);
        return (DBFault);
    }
    if (dateName != (char *) NULL) {
        if ((dateFLD = datTBL->Field(dateName)) == (DBObjTableField *) NULL) {
            CMmsgPrint(CMmsgAppError, "Invalid date field [%s] in: %d %d", dateName, __FILE__, __LINE__);
            return (DBFault);
        }
    }
...
DBInt RGlibGridReclassDiscrete(DBObjData *srcData, char *srcFieldName, DBObjData *dstData) {
    ...
    if ((srcField = srcItemTable->Field(srcFieldName)) == (DBObjTableField *) NULL) {
        CMmsgPrint(CMmsgUsrError, "Invalid source field [%s]!", srcFieldName);
        return (DBFault);
    }
...
DBInt RGlibGridReclassContinuous(DBObjData *srcData, char *srcFieldName, DBObjData *dstData) {
    ...
    if ((srcField = srcItemTable->Field(srcFieldName)) == (DBObjTableField *) NULL) {
        CMmsgPrint(CMmsgUsrError, "Invalid Field [%s]!", srcFieldName);
        return (DBFault);
    }
...
DBInt RGlibGridZoneHistogram(DBObjData *zGrdData, DBObjData *cGrdData, DBObjData *tblData) {
    ...
    if ((cellNumARR = (DBInt *) calloc(zoneTable->ItemNum() * classTable->ItemNum(), sizeof(DBInt))) ==
        (DBInt *) NULL) {
    ...
    if ((areaARR = (DBFloat *) calloc(zoneTable->ItemNum() * classTable->ItemNum(), sizeof(DBFloat))) ==
        (DBFloat *) NULL) {
        CMmsgPrint(CMmsgSysError, "Memory Allocation Error in: %s %d", __FILE__, __LINE__);
        free(cellNumARR);
        delete zGrdIF;
    ...
    outTable->AddField(
            outCLayerIDFLD = new DBObjTableField("CaregoryLayerID", DBTableFieldInt, "%4d", sizeof(DBShort)));
    outTable->AddField(
            outCLayerNameFLD = new DBObjTableField("CategoryLayerName", DBTableFieldString, "%s", DBStringLength));
            ...
            for (zoneID = 0; zoneID < zoneTable->ItemNum(); ++zoneID) {
                ...
                for (classID = 0; classID < classTable->ItemNum(); ++classID)
                    if (cellNumARR[zoneID * classTable->ItemNum() + classID] > 0) {
                        ...
                        outPercentFLD->Float(outRec,
                                             areaARR[zoneID * classTable->ItemNum() + classID] * 100.0 / totArea);
        ...
        for (wLayerID = 0; wLayerID < wGrdIF->LayerNum(); ++wLayerID) {
            ...
            for (pos.Row = 0; pos.Row < zGrdIF->RowNum(); ++pos.Row) {
                ...
                for (pos.Col = 0; pos.Col < zGrdIF->ColNum(); ++pos.Col) {
                    ...
                    tmpSumValSqrFLD->Float(zoneRec,
                                           tmpSumValSqrFLD->Float(zoneRec) + value * value * zGrdIF->CellArea(pos));
...
DBInt RGlibGridNoNegatives(DBObjData *grdData, DBInt setZero) {
    ...
    for (layerID = 0; layerID < gridIF->LayerNum(); ++layerID) {
        ...
        for (pos.Row = 0; pos.Row < gridIF->RowNum(); ++pos.Row) {
            ...
            for (pos.Col = 0; pos.Col < gridIF->ColNum(); ++pos.Col)
                if (gridIF->Value(layerRec, pos, &value)) if (value < 0.0)
                    gridIF->Value(layerRec, pos, setZero ? 0.0 : gridIF->MissingValue());
        }
        gridIF->RecalcStats(layerRec);
    }
    Stop:
    ...
    for (pos.Row = 0; pos.Row < srcIF->RowNum(); ++pos.Row) {
        ...
        for (pos.Col = 0; pos.Col < srcIF->ColNum(); ++pos.Col) {
            ...
            if (obsNum > 0) {
                ...
                statIF->Value(statIF->Layer(layerID++), pos,
                              (max - min) / (fabs(max) + fabs(min) > CMmathEpsilon ? fabs(max) + fabs(min) : 1.0));
                ...
                if (obsNum > DBMathRegressionMin) {
                    ...
                    cor = (sumXY - sumX * sumY / obsNum) /
                          sqrt((sumX2 - sumX * sumX / obsNum) * (sumY2 - sumY * sumY / obsNum));
                    se2 = (sumY2 - 2.0 * b1 * sumXY - 2.0 * b0 * sumY + 2.0 * b1 * b0 * sumX + b1 * b1 * sumX2 +
                           obsNum * b0 * b0);
                    R2 = 1.0 - se2 / (sumY2 - sumY * sumY / obsNum);
                    S = sqrt(se2 / (obsNum - 2.0));
                    b0Bound =
                            DBMathStudentsT(obsNum - 2.0) * sqrt(sumX2 / (obsNum * (sumX2 - sumX * sumX / obsNum))) * S;
                    b1Bound =
                            DBMathStudentsT(obsNum - 2.0) * sqrt(se2 / (obsNum - 2.0) / (sumX2 - sumX * sumX / obsNum));
...
DBInt RGlibGridBivarCellStats(DBObjData *xSrcData, DBObjData *ySrcData, DBObjData *statData) {
    ...
    if (xSrcGridIF->LayerNum() < DBMathRegressionMin) {
        CMmsgPrint(CMmsgUsrError, "Insufficent number [%d] of layers!", xSrcGridIF->LayerNum());
        goto Stop;
    }
        ...
        for (pos.Col = 0; pos.Col < tsGridIF->ColNum(); pos.Col++) {
            for (step = 0; step < stepNum; ++step) {
                switch (aggrType) {
                    default:
                        ...
                        obsNum[step] = 0;
                        break;
                    case RGlibAggrMinimum:
                        sum[step] = HUGE_VAL;
                        obsNum[step] = 0;
                        break;
                    case RGlibAggrMaximum:
                        sum[step] = -HUGE_VAL;
                        obsNum[step] = 0;
                        break;
                }
            ...
            for (tsLayerID = 0; tsLayerID < tsGridIF->LayerNum(); ++tsLayerID) {
                ...
                if (tsGridIF->Value(layerRec, pos, &value)) {
                    switch (timeStep) {
                        case DBTimeStepHour:
                            step = date.HoursAD() - sDate.HoursAD();
                            break;
                        case DBTimeStepDay:
                            step = date.DaysAD() - sDate.DaysAD();
                            break;
                        case DBTimeStepMonth:
                            step = date.MonthsAD() - sDate.MonthsAD();
                            break;
                        case DBTimeStepYear:
                            step = date.YearsAD() - sDate.YearsAD();
                            break;
                    }
                    switch (aggrType) {
                        default:
                            ...
                            break;
                        case RGlibAggrMinimum:
                            sum[step] = sum[step] < value ? sum[step] : value;
                            break;
                        case RGlibAggrMaximum:
                            sum[step] = sum[step] > value ? sum[step] : value;
                            break;
...
DBInt RGlibSeasonMean(DBObjData *tsData, DBObjData *data, DBInt seasonLen, DBInt offset, DBInt doSum) {
    ...
    for (pos.Row = 0; pos.Row < tsGridIF->RowNum(); pos.Row++) {
        if (DBPause(100 * pos.Row / tsGridIF->RowNum())) goto Stop;
        for (pos.Col = 0; pos.Col < tsGridIF->ColNum(); pos.Col++) {
            for (season = 0; season < seasonNum; ++season) {
                sum[season] = 0.0;
                obsNum[season] = 0;
            }
            for (tsLayerID = 0; tsLayerID < tsGridIF->LayerNum(); ++tsLayerID) {
...
DBInt RGlibGridSampling(DBObjData *dbData, DBObjData *grdData, DBObjData *tblData) {
    ...
    switch (grdData->Type()) {
        case DBTypeGridContinuous: {
            ...
            newField = new DBObjTableField(grdData->Document(DBDocSubject), DBTableFieldFloat, gridIF->ValueFormat(),
                                           sizeof(DBFloat4));
                ...
                for (layerID = 0; layerID < gridIF->LayerNum(); ++layerID) {
                    ...
                    if ((grdRec = gridIF->GridItem(layerRec, coord)) != (DBObjRecord *) NULL) {
                        ...
                        for (fieldID = firstFieldID + 1; fieldID < table->FieldNum(); fieldID++) {
                            ...
                            switch (field->Type()) {
                                case DBTableFieldString:
                                    newField->String(tblRec, field->String(grdRec));
                                    break;
                                case DBTableFieldInt:
                                    newField->Int(tblRec, field->Int(grdRec));
                                    break;
                                case DBTableFieldFloat:
                                    newField->Float(tblRec, field->Float(grdRec));
                                    break;
                                case DBTableFieldDate:
                                    newField->Date(tblRec, field->Date(grdRec));
                                    break;
                            }
                        }
                    }
                        ...
                        for (fieldID = firstFieldID + 1; fieldID < table->FieldNum(); fieldID++) {
                            ...
                            switch (field->Type()) {
                                case DBTableFieldString:
                                    ...
                                    break;
                                case DBTableFieldInt:
                                    newField->Int(tblRec, newField->IntNoData());
                                    break;
                                case DBTableFieldFloat:
                                    newField->Float(tblRec, newField->FloatNoData());
                                    break;
                                case DBTableFieldDate:
                                    newField->Date(tblRec, newField->DateNoData());
                                    break;
                            }
                        }
                    }
...
void RGlibGridSampling(DBObjData *splData, DBObjData *grdData, bool interpolate, bool netMode) {
    ...
    if (layerNum < 1) {
        CMmsgPrint(CMmsgAppError, "No Layer to Process!");
        delete gridIF;
        return;
    }

    if (splData->Type() == DBTypeVectorPoint) {
        pntIF = new DBVPointIF(splData);
        tableName = DBrNItems;
    }
    else {
        netIF = new DBNetworkIF(splData);
        tableName = DBrNCells;
    }
    ...
    switch (grdData->Type()) {
        case DBTypeGridContinuous: {
            ...
            for (layerID = 0; layerID < gridIF->LayerNum(); ++layerID) {
                ...
                if ((newField = table->Field(layerRec->Name())) == (DBObjTableField *) NULL) {
                    table->AddField(newField = new DBObjTableField(layerRec->Name(), DBTableFieldFloat, "%10.3f",
                                                                   sizeof(DBFloat4)));
        ...
        case DBTypeGridDiscrete: {
            ...
            if ((newField = table->Field("CategoryName")) == (DBObjTableField *) NULL)
                table->AddField(
                        newField = new DBObjTableField("CategoryName", DBTableFieldString, "%s", DBStringLength));
            for (recordID = 0; recordID < table->ItemNum(); recordID++) {
                record = table->Item(recordID);
                if (pntIF != (DBVPointIF *) NULL) coord = pntIF->Coordinate(record);
                else coord = netMode ? netIF->Center(record) : netIF->Center(record) + netIF->Delta(record);
            ...
            for (field = fields->First(); field != (DBObjTableField *) NULL; field = fields->Next())
                if (DBTableFieldIsVisible(field)) {
                    ...
                    for (recordID = 0; recordID < table->ItemNum(); recordID++) {
                        record = table->Item(recordID);
                        DBPause((field->RowID() * table->ItemNum() + recordID) * 100 /
                                (fields->ItemNum() * table->ItemNum()));
                        if (pntIF != (DBVPointIF *) NULL) coord = pntIF->Coordinate(record);
                        else coord = netMode ? netIF->Center(record) : netIF->Center(record) + netIF->Delta(record);
                        ...
                        if ((grdRec = gridIF->GridItem(coord)) != (DBObjRecord *) NULL)
                            switch (field->Type()) {
                                default:
                                case DBTableFieldString:
                                    newField->String(record, field->String(grdRec));
                                    break;
                                case DBTableFieldInt:
                                    newField->Int(record, field->Int(grdRec));
                                    break;
                                case DBTableFieldFloat:
                                    newField->Float(record, field->Float(grdRec));
                                    break;
                                case DBTableFieldDate:
                                    newField->Date(record, field->Date(grdRec));
                                    break;
                            }
                    }
                }
